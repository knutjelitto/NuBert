...
namespace Pliant.Grammars
{
    ...
    public struct Interval : IComparable<Interval>
    {
        ...
        public static IReadOnlyList<Interval> Group(IReadOnlyList<Interval> input)
        {
            ...
            var sortedIntervals = pool.AllocateAndClear();
            sortedIntervals.AddRange(input);
            sortedIntervals.Sort();

            var intervalList = pool.AllocateAndClear();
        ...
        {
            var compareMin = this.Min.CompareTo(other.Min);
            if (compareMin != 0)
            {
                return compareMin;
            }
        ...
        public bool Touches(Interval other)
        {
            ...
            if (this.Min - other.Max == 1)
            {
                return true;
            }

            if (other.Min - this.Max == 1)
            {
                return true;
            }
        ...
        private static readonly Interval[] EmptyList = { };
    }
}